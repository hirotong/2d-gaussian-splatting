...
class EnvironmentLight(torch.nn.Module):
  ...
  #     return rgb, extras
  def shade(self, gb_pos, gb_normal, kd, kr, ks, metallic, albedo, view_pos, specular=True, mode="gs"):
    if mode == "pbr":
      return self.shade_pbr(gb_pos, gb_normal, kr, metallic, albedo, view_pos, specular)
    elif mode == "gs":
      return self.shade_gs(gb_pos, gb_normal, kd, kr, ks, view_pos, specular)
    else:
      raise ValueError("Unknown shading mode")

  def shade_gs(self, gb_pos, gb_normal, kd, kr, ks, view_pos, specular=True):
    # (H, W, N, C)
    wo = util.safe_normalize(view_pos - gb_pos)
...
#   def shade_gs(self, gb_pos, gb_normal, kd, metallic, kr, albedo, view_pos, specular=True):
#     # (H, W, N, C)
#     wo = util.safe_normalize(view_pos - gb_pos)
...
#     return rgb, extras
  ...
  def shade_pbr(self, gb_pos, gb_normal, kr, metallic, albedo, view_pos, specular=True):
    """
