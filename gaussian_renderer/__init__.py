...
from utils.point_utils import depth_to_normal, normal_from_depth_image
...
def rendered_world2cam(viewpoint_cam, normal, alpha, bg_color):
  ...
  normal_cam = normal_cam.reshape(H, W, 3).permute(2, 0, 1)  # (3, H, W)
...
def normalize_normal_inplace(normal, alpha):
  ...
  fg_mask = (alpha > 0.0).repeat(3, 1, 1)
...
def render(
  ...
  if pipe.compute_cov3D_python:
    ...
    ndc2pix = (
      torch.tensor([[W / 2, 0, 0, (W - 1) / 2], [0, H / 2, 0, (H - 1) / 2], [0, 0, far - near, near], [0, 0, 0, 1]])
      .float()
      .cuda()
      .T
    )
    ...
    if pipe.convert_SHs_python:
      if pipe.brdf:
        ...
        if pipe.brdf_mode == "envmap":
          ...
          normal_axis, delta_normal = pc.get_normal(dir_pp_normalized, return_delta=True)
  ...
  if not speed:
    ...
    if pipe.brdf:
      ...
      normal_axis_normed = normal_axis * 0.5 + 0.5  # [-1, 1] -> [0, 1]
      render_extras.update({"normal_axis": normal_axis_normed})
      if delta_normal_norm is not None:
        render_extras.update({"delta_normal_norm": delta_normal_norm})
        
    out_extras = {}
    for k in render_extras.keys():
      if render_extras[k] is not None:
        image = rasterizer(
          means3D=means3D,
          means2D=means2D,
          shs=None,
    ...
    for k in ["normal", "normal_axis"] if debug else ["normal", "normal_axis"]:
  ...
  normalize_normal_inplace(render_normal, render_alpha)
  ...
  if pipe.brdf:
    normalize_normal_inplace(out_extras["normal_axis"], render_alpha)
    # out_extras["normal_cam"] = rendered_world2cam(viewpoint_camera, out_extras["normal"], render_alpha, bg_color).squeeze(0)
    # out_extras["normal_cam"] = (out_extras["normal"].permute(1, 2, 0) @ (viewpoint_camera.world_view_transform[:3, :3].T)).permute(2, 0, 1)
    # out_extras["normal_cam"] = out_extras["normal"]
  rets.update(
    {
...
def render_lighting(pc: GaussianModel, resolution=(512, 1024), sample_index=None):
