...
def render(
    viewpoint_camera: Camera, pc: GaussianModel, pipe, bg_color: torch.Tensor, scaling_modifier=1.0, override_color=None
):
    """
    Render the scene.
    ...
    rendered_confidence, _, _ = rasterizer(
        means3D=means3D,
        means2D=means2D,
        shs=None,
        colors_precomp=confidence.repeat(1, 3),
        opacities=opacity,
        scales=scales,
        rotations=rotations,
        cov3D_precomp=cov3D_precomp,
    ...
    rets = {
        ...
        "radii": radii,
        "rend_confidence": rendered_confidence[0:1],
    }
