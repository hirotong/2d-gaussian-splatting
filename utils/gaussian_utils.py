...
class GaussianTransformUtils:
    ...
    def transform_shs(features, rotation_matrix):
        ...
        ## rotate shs
        P = torch.tensor(
            [[0, 0, 1], [1, 0, 0], [0, 1, 0]], dtype=shs_feat.dtype, device=shs_feat.device
        )  # switch axes: yzx -> xyz
    ...
    @classmethod
    def transform_xyz(cls, xyz, rotation_matrix):
        return torch.matmul(xyz, rotation_matrix.T)
    
    @classmethod
    def transform_rotation(cls, rotations, rotation_matrix):
        # rotate via quaternion
        rotations = torch.nn.functional.normalize(
            cls.quat_multiply(
                rotations,
                torch.tensor([rotmat2qvec(rotation_matrix.cpu().numpy())]).to(rotations),
            )
        )
    ...
    def rotate_by_wxyz_quaternions(cls, xyz, rotations, features, quaternions: torch.tensor):
        if torch.all(quaternions == 0.0) or torch.all(
            quaternions
            == torch.tensor(
                [1.0, 0.0, 0.0, 0.0],
