...
class GaussianExtractor(object):
  ...
  def reconstruction(self, viewpoint_stack):
    ...
    for i, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack), desc="reconstruct radiance fields"):
      ...
      if self.gaussians.brdf:
        ...
        diffuse_color = render_pkg["diffuse_color"]
        specular_color = render_pkg["specular_color"]
        normal_cam = render_pkg["normal_cam"]
        if self.pipe.linear:
          albedo = linear2srgb(albedo)
          diffuse_color = linear2srgb(diffuse_color)
          specular_color = linear2srgb(specular_color)
        self.metallics.append(metallic.cpu())
        self.speculars.append(specular.cpu())
        self.roughness.append(roughness.cpu())
        self.albedos.append(albedo.cpu())
        self.diffuse_maps.append(diffuse_color.cpu())
        self.specular_maps.append(specular_color.cpu())
    ...
    if self.gaussians.brdf:
      self.metallics = torch.stack(self.metallics, dim=0)
      self.speculars = torch.stack(self.speculars, dim=0)
      self.roughness = torch.stack(self.roughness, dim=0)
      self.albedos = torch.stack(self.albedos, dim=0)
      self.diffuse_maps = torch.stack(self.diffuse_maps, dim=0)
      self.specular_maps = torch.stack(self.specular_maps, dim=0)
      self.normals_cam = torch.stack(self.normals_cam, dim=0)
  ...
  def extract_mesh_unbounded(self, resolution=1024):
    ...
    def compute_sdf_perframe(i, points, depthmap, rgbmap, normalmap, viewpoint_cam):
      ...
      sampled_depth = torch.nn.functional.grid_sample(
        depthmap.cuda()[None], pix_coords[None, None], mode="bilinear", padding_mode="border", align_corners=True
      ).reshape(-1, 1)
      sampled_rgb = (
        torch.nn.functional.grid_sample(
          rgbmap.cuda()[None], pix_coords[None, None], mode="bilinear", padding_mode="border", align_corners=True
        )
        .reshape(3, -1)
        .T
      )
      sampled_normal = (
        torch.nn.functional.grid_sample(
          normalmap.cuda()[None], pix_coords[None, None], mode="bilinear", padding_mode="border", align_corners=True
        )
        .reshape(3, -1)
        .T
      )
  ...
  def export_image(self, path):
    ...
    for idx, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack), desc="export images"):
      ...
      save_img_u8(
        self.rgbmaps[idx].permute(1, 2, 0).cpu().numpy(), os.path.join(render_path, "{0:05d}".format(idx) + ".png")
      )
      ...
      if self.gaussians.brdf:
        save_img_u8(
          self.metallics[idx].permute(1, 2, 0)[..., 0].cpu().numpy(),
          os.path.join(vis_path, "metallic_{0:05d}".format(idx) + ".png"),
        )
        save_img_u8(
          self.speculars[idx].permute(1, 2, 0)[..., 0].cpu().numpy(),
          os.path.join(vis_path, "specular_{0:05d}".format(idx) + ".png"),
        )
        save_img_u8(
          self.roughness[idx].permute(1, 2, 0)[..., 0].cpu().numpy(),
          os.path.join(vis_path, "roughness_{0:05d}".format(idx) + ".png"),
        )
        save_img_u8(
          self.albedos[idx].permute(1, 2, 0).cpu().numpy(),
          os.path.join(vis_path, "albedo_{0:05d}".format(idx) + ".png"),
        )
        save_img_u8(
          self.diffuse_maps[idx].permute(1, 2, 0).cpu().numpy(),
          os.path.join(vis_path, "diffuse_{0:05d}".format(idx) + ".png"),
        )
        save_img_u8(
          self.specular_maps[idx].permute(1, 2, 0).cpu().numpy(),
          os.path.join(vis_path, "specular_{0:05d}".format(idx) + ".png"),
        )
        save_img_u8(
          self.normals_cam[idx].permute(1, 2, 0).cpu().numpy() * 0.5 + 0.5,
          os.path.join(vis_path, "normal_cam_{0:05d}".format(idx) + ".png"),
        )