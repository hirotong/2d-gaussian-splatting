...
class RayTracer:
    ...
    @torch.no_grad()
    def trace_visibility_2dgs(self, rays_o, rays_d, means3D, cov3D_inv, opacity, normals):
        contrib, opa = _C.trace_bvh_opacity_2dgs(self.tree, self.aabb, rays_o, rays_d, means3D, cov3D_inv, opacity, normals)
        
        return {
            "visibility": opa.unsqueeze(-1),
            "contribute": contrib.unsqueeze(-1),
        }