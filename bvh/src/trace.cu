...
std::tuple<int32_t, thrust::device_vector<int32_t>,
           thrust::device_vector<float3>, thrust::device_vector<int32_t>>
trace_bvh_cuda(int32_t num_rays, int32_t *nodes, float *aabbs, float3 *rays_o,
               float3 *rays_d, float3 *means3D, float *covs3D, float *opacities,
  ...
  // count how many leave nodes are intersected by each ray and store the result
  // in num_contributes
  thrust::for_each(thrust::device, thrust::make_counting_iterator<int32_t>(0),
                   thrust::make_counting_iterator<int32_t>(num_rays),
                   [nodes, aabbs_internal, rays_o, rays_d,
                    num_contributes] __device__(int32_t idx) {
                     ...
                     while (!stack_device.empty()) {
                       ...
                       if (node[4] <= 4) {
                         ...
                         float2 interection_l =
                             ray_intersects(aabbs_internal[lid], ray_o, ray_d);
                         float2 interection_r =
                             ray_intersects(aabbs_internal[rid], ray_o, ray_d);
  ...
  thrust::device_ptr<int32_t> num_contributes_ptr =
      thrust::device_pointer_cast(num_contributes);

  thrust::inclusive_scan(num_contributes_ptr, num_contributes_ptr + num_rays,
                         ray_offsets_vec.begin());
  int32_t *ray_offsets = thrust::raw_pointer_cast(ray_offsets_vec.data());
  int32_t num_rendered;
  cudaMemcpy(&num_rendered, ray_offsets + num_rays - 1, sizeof(int32_t),
  ...
  //     cudaEventSynchronize(stop);
  //     cudaEventElapsedTime(&milliseconds, start, stop);
  //     std::cout << "inclusive_scan time: " << milliseconds << " ms" <<
  //     std::endl;
  cudaEventRecord(start);
  ...
  thrust::for_each(
      ...
      [nodes, aabbs_internal, rays_o, rays_d, num_contributes, ray_offsets,
       ray_list_key, point_list, ray_id_list, means3D, covs3D,
       position_list] __device__(uint32_t idx) {
          ...
          if (node[4] <= 4) {
            ...
            while (!stack2.empty()) {
              ...
              if (node[3] >= 0) {
                ...
                // float t = ray_intersects(means3D[object_id], covs3D +
                // object_id*6, ray_o, ray_d);
  ...
  thrust::stable_sort_by_key(
      ray_list_key_vec.begin(), ray_list_key_vec.end(),
      thrust::make_zip_iterator(thrust::make_tuple(point_list_vec.begin(),
  ...
  //     cudaEventRecord(start);
  return std::make_tuple(num_rendered, point_list_vec, position_list_vec,
                         ray_id_list_vec);
}

void trace_bvh_opacity_cuda(int32_t num_rays, int32_t *nodes, float *aabbs,
                            float3 *rays_o, float3 *rays_d, float3 *means3D,
                            float *covs3D, float *opacities, float3 *normals,
                            int32_t *num_contributes, float *rendered_opacity) {
  //         cudaEvent_t start, stop;
  //         cudaEventCreate(&start);
  //         cudaEventCreate(&stop);
  //         cudaEventRecord(start);
  ...
  auto *aabbs_internal = reinterpret_cast<aabb_type *>(aabbs);
  thrust::for_each(
      thrust::device, thrust::make_counting_iterator<int32_t>(0),
      thrust::make_counting_iterator<int32_t>(num_rays),
      [nodes, aabbs_internal, rays_o, rays_d, num_contributes, means3D, covs3D,
       opacities, normals, rendered_opacity] __device__(int32_t idx) {
        IndexStack<int32_t> stack_device;
        stack_device.push(0);
        int32_t count = 0;
        float3 ray_o = rays_o[idx], ray_d = rays_d[idx];
          ...
          if (node[4] <= 1) { // 8
            ...
            while (!stack_device2.empty()) {
              ...
              if (node2[4] > 1) {
                ...
                if (normal.x * ray_d.x + normal.y * ray_d.y +
                        normal.z * ray_d.z >
                    0)
                  continue;

                float t = ray_intersects(means3D[object_id2],
                                         covs3D + object_id2 * 6, ray_o, ray_d);
                if (t < 0.01) {
                  continue;
                }
                float3 pos = {
                ...
                float power = gaussian_fn(means3D[object_id2], pos,
                                          covs3D + object_id2 * 6);
                if (power > 0)
                  continue;
                count += 1;
...
void trace_bvh_opacity_2dgs_cuda(int32_t num_rays, int32_t *nodes, float *aabbs,
        ...
        while (!stack_device.empty()) {
          ...
          if (node[4] <= 1) {
            ...
            while (!stack_device2.empty()) {
              ...
              int32_t *node2 = nodes + node_id2 * 5;
              if (node2[4] > 1) {
                // If the node is not a leaf node, push its
                ...
                float t =
                    ray_intersects(means3D[object_id2], normal, ray_o, ray_d);
                if (t < 0.01) {
                  continue;
                }
                float3 pos = {
                ...
                float power = twoD_gaussian_fn(means3D[object_id2], pos,
                                               covs3D + object_id2 * 16);
                if (power > 0)
                  continue;
                count += 1;
                float alpha = opacities[object_id2] * __expf(power);
                ray_opacity *= (1 - alpha);
                if (ray_opacity < 0.9) {
                  rendered_opacity[idx] = 0.0f; // early stop?
                  return;
                }
              }
            }
          } else {
            // If the node is not a leaf node, push its children to the stack
            int32_t lid = node[1], rid = node[2];
            float2 intersection_l =
                ray_intersects(aabbs_internal[lid], ray_o, ray_d);
            float2 intersection_r =
                ray_intersects(aabbs_internal[rid], ray_o, ray_d);
            if (intersection_l.y > intersection_r.y) {
              if (intersection_l.y > 0) {
                stack_device.push(lid);
              }
              if (intersection_r.y > 0) {
                stack_device.push(rid);
              } else {
                if (intersection_r.y > 0) {
                  stack_device.push(rid);
