...
create_bvh(const torch::Tensor &means3D, const torch::Tensor &scales,
           const torch::Tensor &rotations, const torch::Tensor &nodes,
  ...
  torch::Tensor mortons =
      torch::zeros({P}, means3D.options().dtype(torch::kLong));

  construct_bvh(P, means3D.contiguous().data_ptr<float>(),
...
trace_bvh(const torch::Tensor &nodes, const torch::Tensor &aabbs,
  ...
  auto result =
      trace_bvh_cuda(num_rays, nodes.contiguous().data_ptr<int32_t>(),
                     aabbs.contiguous().data_ptr<float>(),
  ...
  if (num_rendered == 0) {
    ...
    return std::make_tuple(num_contributes, point_list_tensor,
                           position_list_tensor, ray_id_list_tensor);
  }
  int32_t *point_list = thrust::raw_pointer_cast(point_list_vec.data());
  int32_t size = point_list_vec.size();
  torch::Tensor point_list_tensor =
      torch::from_blob(point_list, {size, 1}, int_opts);
  point_list_tensor = point_list_tensor.clone();

  float *position_list =
      (float *)thrust::raw_pointer_cast(position_list_vec.data());
  torch::Tensor position_list_tensor =
      torch::from_blob(position_list, {size, 3}, float_opts);
  position_list_tensor = position_list_tensor.clone();

  int32_t *ray_id_list = thrust::raw_pointer_cast(ray_id_list_vec.data());
  torch::Tensor ray_id_list_tensor =
      torch::from_blob(ray_id_list, {size, 1}, int_opts);
...
trace_bvh_opacity(const torch::Tensor &nodes, const torch::Tensor &aabbs,
  ...
  trace_bvh_opacity_cuda(num_rays, nodes.contiguous().data_ptr<int32_t>(),
                         aabbs.contiguous().data_ptr<float>(),
                         (float3 *)rays_o.contiguous().data_ptr<float>(),
                         (float3 *)rays_d.contiguous().data_ptr<float>(),
                         (float3 *)means3D.contiguous().data_ptr<float>(),
...
std::tuple<torch::Tensor, torch::Tensor> trace_bvh_opacity_2dgs(
    const torch::Tensor &nodes, const torch::Tensor &aabbs,
    const torch::Tensor &rays_o, const torch::Tensor &rays_d,
    const torch::Tensor &means3D, const torch::Tensor &covs3D,
  ...
  trace_bvh_opacity_2dgs_cuda(num_rays, nodes.contiguous().data_ptr<int32_t>(),
                              aabbs.contiguous().data_ptr<float>(),
                              (float3 *)rays_o.contiguous().data_ptr<float>(),
                              (float3 *)rays_d.contiguous().data_ptr<float>(),
                              (float3 *)means3D.contiguous().data_ptr<float>(),
