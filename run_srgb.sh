...
for scene in ${scenes[@]}; do
    ...
    (
        python train.py -s data/refnerf/${scene} --eval -m output/refnerf/${scene}/pbr_gs_mtx -w --brdf_dim 0 --sh_degree -1 --lambda_predicted_normal 2e-1 --brdf_env 512 --shading gs
        python render.py -m output/refnerf/${scene}/pbr_gs_mtx --brdf_dim 0 --sh_degree -1 --brdf_mode envmap --brdf_env 512 --shading gs
        python metrics.py -m output/refnerf/${scene}/pbr_gs_mtx
    )
    wait
    # (
