...
def get_visual_hull2(N, bbox, scene_info, cam_center):
    ...
    coords += cam_center[None, None, None, :]

    volume = -np.ones_like(xx)
    Ks = scene_info.Ks
    Ts = scene_info.Ts  # w2c

    masks = scene_info.masks
    image_size = masks[0].shape[1:]

    for i, (mask, K, T) in enumerate(zip(masks, Ks, Ts)):
        print(f"Processing {i+1}/{len(masks)} images")
        mask = mask.permute(1, 2, 0).cpu().numpy()
        K = K.cpu().numpy()
        T = T.cpu().numpy()
        c2w = np.linalg.inv(T)

        coords_homo = np.concatenate([coords, np.ones((N, N, N, 1))], axis=-1)
        coords_cam = coords_homo @ T.transpose()
        coords_cam = coords_cam[..., :3]

        # assert np.all(coords_cam[..., 2] > 0)
        valid_z = (coords_cam[..., 2] > 0)

        coords_img = coords_cam @ K.transpose()

        coords_uv = coords_img[..., :2]
        coords_uv /= coords_img[..., -1:]

        x_ind = np.logical_and(coords_uv[..., 0] >= 0, coords_uv[..., 0] < image_size[1] - 0.5)
        y_ind = np.logical_and(coords_uv[..., 1] >= 0, coords_uv[..., 1] < image_size[0] - 0.5)
        im_ind = np.logical_and(x_ind, y_ind)
        im_ind = np.logical_and(im_ind, valid_z)

        img_uv = np.round(coords_uv[im_ind]).astype(np.int32)
        mask_ind = mask[img_uv[..., 1], img_uv[..., 0]].squeeze()
        ...
        print(f"Occupied voxel {im_ind.sum() / N**3 * 100:.2f}%")

    vertex, faces, normals, _ = measure.marching_cubes(volume, 0)

    vertex[..., 0] = vertex[..., 0] / (N - 1) * (xe - xs) + xs + cam_center[0]
    vertex[..., 1] = vertex[..., 1] / (N - 1) * (ye - ys) + ys + cam_center[1]
    vertex[..., 2] = vertex[..., 2] / (N - 1) * (ze - zs) + zs + cam_center[2]

    mesh = o3d.geometry.TriangleMesh()
    mesh.vertices = o3d.utility.Vector3dVector(vertex)
    mesh.triangles = o3d.utility.Vector3iVector(faces)
    mesh.vertex_normals = o3d.utility.Vector3dVector(normals)
    mesh.paint_uniform_color(np.array([0, 0, 0]))
...
def get_visual_hull(N, bbox, scene_info, cam_center):
    ...
    for h_id in trange(N):
        i, j = torch.meshgrid(torch.linspace(xs, xe, N).cuda(), torch.linspace(ys, ye, N).cuda())
        i, j = i.t(), j.t()
        ...
        valid_x_y_mask = (
            (uv[..., 0] > 0)
            & (uv[..., 0] < cam_info.image_width)
            & (uv[..., 1] > 0)
            & (uv[..., 1] < cam_info.image_height)
        )
        ...
        result = F.grid_sample(images.float(), uv, padding_mode="zeros", align_corners=False).permute(
            0, 2, 3, 1
        )  # N, 100, 100, 3
        # sample mask
        result_mask = F.grid_sample(masks.float(), uv, padding_mode="zeros", align_corners=False).permute(
            0, 2, 3, 1
        )  # N, 100, 100, 1
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        default="sparse_nerf_datasets/sparse_omni3d_undistorted/backpack_016",
        help="data directory, we only support colmap type data, kitchen, garden",
    )
    parser.add_argument("--cube_size", type=float, default=2.0, help="size of the cube in meters")
    ...
    if os.path.exists(os.path.join(args.data_dir, "sparse")):
        scene_info = sceneLoadTypeCallbacks["Colmap"](args.data_dir, "images", True, extra_opts=extra_opts)
    elif os.path.exists(os.path.join(args.data_dir, "transforms_train.json")):
        print("Found transforms_train.json file, assuming Blender data set!")
        scene_info = sceneLoadTypeCallbacks["Blender"](args.data_dir, True, True)
    ...
    for cam_info in camlist:
        ...
        masks.append(cam_info.gt_alpha_mask)
    ...
    o3d_mesh, bbox = get_visual_hull2(args.voxel_num, init_bbox, scene_info, np.array([0, 0, 0]))
    
    o3d.io.write_triangle_mesh(os.path.join(args.data_dir, "visual_hull_bound.ply"), o3d_mesh)
    ...
    o3d_mesh, bbox_new = get_visual_hull2(256, [enlarged_bbox_min, enlarged_bbox_max], scene_info, np.array([0, 0, 0]))


    pcd = o3d_mesh.sample_points_poisson_disk(args.num_points, init_factor=5)
    ...
    o3d.io.write_triangle_mesh(os.path.join(args.data_dir, "visual_hull.ply"), o3d_mesh)
    o3d.io.write_point_cloud(os.path.join(args.data_dir, "visual_hull_points.ply"), pcd)
    ...
    # if not args.not_vis:
        ...
        world_coord.points = o3d.utility.Vector3dVector(
            np.array([[0, 0, 0], [2, 0, 0], [0, 0, 0], [0, 2, 0], [0, 0, 0], [0, 0, 2]])
        )
